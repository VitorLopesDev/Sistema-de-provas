...
import { Clock, AlertTriangle, CheckCircle, ChevronRight, ArrowLeft, Eye, Zap, Calendar, Archive } from "lucide-react"
...
function ListaProvas({ onVerDetalhe }) {
      ...
      <div style={s.card}>
        ...
        <div style={s.cardRodape}>
          <div style={s.estatisticas}>
            <div style={s.stat}>
              <CheckCircle size={12} color="#1F5C6B" />
              <span>{enviados}/{prova.totalAlunos}</span>
            </div>
            {comAlerta > 0 && (
              <div style={s.stat}>
                <AlertTriangle size={12} color="#B23A3A" />
                <span>{comAlerta}</span>
              </div>
            )}
          </div>
  ...
  return (
    <div>
      ...
      <Secao titulo="Ativas"     provas={ativas}     icone={Zap}      corIcone="1F5C6B" />
      <Secao titulo="Agendadas"  provas={agendadas}  icone={Calendar} corIcone="#040220" />
      <Secao titulo="Encerradas" provas={encerradas} icone={Archive}  corIcone="#5F5E5A" />
    </div>
  )
}
